...
| Reuses TF providers | ✅ spawn | ✅ native | ✅ via TF | ✅ *branch* | ✅ bridge |
| Outputs feed back into config | ✅ | ⚠️ | ❌ | ⚠️ | ✅ |
| OS build + cloud in one expression | ✅ | ❌ | ❌ | ⚠️ | ❌ |
...
Different choices, both defensible:

::: {.small-table}
| | NixOps 4 (`tfproto`) | Nivis |
|---|---|---|
| Adapter | Rust | Go |
| Provider comes from | a Nix-packaged provider | the OpenTofu registry, checksum-verified |
| Schema → Nix | IFD, at eval time | `nivis gen`, generated ahead of time |
| Complex types | `types.raw` for now | schema-derived, nested blocks and all |
:::

::: {.fragment}
If two people independently reach for the same coupling,
the coupling is probably right.
:::
